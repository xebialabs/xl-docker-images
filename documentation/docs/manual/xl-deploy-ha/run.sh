...
docker-compose -f docker-compose-xld-ha.yaml up --scale xl-deploy-master=1 -d || exit
...
# # Cleanup
# docker-compose -f docker-compose-xld-ha.yaml -f docker-compose-xld-ha-workers.yaml down --volumes
